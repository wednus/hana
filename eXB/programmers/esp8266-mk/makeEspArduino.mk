...
UPLOAD_PORT := $(if $(UPLOAD_PORT),$(UPLOAD_PORT),/dev/serial0)
...
BUILD_ROOT ?= ./
BUILD_DIR ?= $(BUILD_ROOT)/build-$(CHIP)
...
ifneq ($(ESPTOOL),)
  ...
  ifeq ($(CHIP),esp8266)
    ESPTOOL_COM = $(ESPTOOL)
    UPLOAD_COM = $(ESPTOOL_PATTERN) -a --before no_reset --after no_reset --no-stub write_flash 0x00000 $(BUILD_DIR)/$(MAIN_NAME).bin
    FS_UPLOAD_COM = $(ESPTOOL_PATTERN) -a --before no_reset --after no_reset --no-stub write_flash $(SPIFFS_START) $(FS_IMAGE)
  endif
endif
...
run: flash
	python -m serial.tools.miniterm --rts=0 --dtr=0 $(UPLOAD_PORT) $(UPLOAD_SPEED)

FLASH_FILE ?= $(BUILD_DIR)/esp_flash.bin
dump_flash:
	echo Dumping flash memory to file: $(FLASH_FILE)
	$(ESPTOOL_PATTERN) --before no_reset --after no_reset --no-stub read_flash 0 $(shell perl -e 'shift =~ /(\d+)([MK])/ || die "Invalid memory size\n";$$mem_size=$$1*1024;$$mem_size*=1024 if $$2 eq "M";print $$mem_size;' $(FLASH_DEF)) $(FLASH_FILE)

dump_fs:
	echo Dumping flash file system to directory: $(FS_REST_DIR)
	-$(ESPTOOL_PATTERN) --before no_reset --after no_reset --no-stub read_flash $(SPIFFS_START) $(SPIFFS_SIZE) $(FS_IMAGE)
	mkdir -p $(FS_REST_DIR)
	echo
	echo == Files ==
	$(RESTSPIFFS_COM)
...
restore_flash:
	...
	$(ESPTOOL_PATTERN) -a --before no_reset --after no_reset --no-stub write_flash 0 $(FLASH_FILE)

erase_flash:
	$(ESPTOOL_PATTERN) --before no_reset --after no_reset --no-stuberase_flash
