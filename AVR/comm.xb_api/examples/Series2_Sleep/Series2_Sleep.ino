...
void sendPacket() {
    ...
    if (xbee.readPacket(5000)) {
    	...
    	if (xbee.getResponse().getApiId() == ZB_TX_STATUS_RESPONSE) {
           ...
           if (txStatus.getDeliveryStatus() == SUCCESS) {
            	// success.  time to celebrate
             	Serial.println("packet was delivered");
           } else {
      ...
      // local xbee is sleeping
       Serial.println("no response -- is local xbee sleeping?");
    }  
}
...
void readPacket() {
  xbee.readPacket();
    ...
    if (xbee.getResponse().isAvailable()) {
        // got something.. print packet to Serial
        Serial.print("API=");
     ...
     Serial.print("XBee error. error code is");
